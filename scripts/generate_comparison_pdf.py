...
if __name__ == "__main__":
    md_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(REPORTS_DIR, "comparison_summary.md")
    output_path = (
        sys.argv[2]
        if len(sys.argv) > 2
        else os.path.join(REPORTS_DIR, "Glas_Intelligence_Scenario_Comparison.pdf")
    )
