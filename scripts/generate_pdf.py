...
def generate_action_plan_section(decision_payload):
    """Generate action plan, monitoring indicators, and time sensitivity HTML."""
    ...
    if actions:
        html_parts.append("<h2>Recommended Actions</h2>")
        html_parts.append('<table><tr><th>Priority</th><th>Action</th><th>Rationale</th><th>Timeline</th></tr>')
        for a in actions:
            priority = a.get("priority", "medium").upper()
            css = "risk-high" if priority in ("CRITICAL", "HIGH") else "risk-medium" if priority == "MEDIUM" else "risk-low"
            html_parts.append(
                f'<tr class="{css}"><td><strong>{priority}</strong></td>'
                f'<td>{a.get("action", "")}</td>'
                f'<td>{a.get("rationale", "")}</td>'
                f'<td>{a.get("timeline", "")}</td></tr>'
            )
        html_parts.append("</table>")

    criteria = decision_payload.get("decision_criteria", [])
    if criteria:
        html_parts.append("<h3>Decision Criteria</h3><ul>")
        for c in criteria:
            html_parts.append(f"<li>{c}</li>")
        html_parts.append("</ul>")

    indicators = decision_payload.get("monitoring_indicators", [])
    if indicators:
        html_parts.append("<h3>Monitoring Indicators</h3>")
        html_parts.append('<table><tr><th>Indicator</th><th>Current State</th><th>Threshold</th><th>Action if Triggered</th></tr>')
        for m in indicators:
            html_parts.append(
                f'<tr><td>{m.get("indicator", "")}</td>'
                f'<td>{m.get("current_state", "")}</td>'
                f'<td>{m.get("threshold", "")}</td>'
                f'<td>{m.get("action_if_triggered", "")}</td></tr>'
            )
        html_parts.append("</table>")

    ts = decision_payload.get("time_sensitivity", "")
    if ts:
        html_parts.append(f'<div class="methodology-box"><strong>Time Sensitivity:</strong> {ts}</div>')

    return "\n".join(html_parts)


def generate_mc_section(monte_carlo):
    """Generate Monte Carlo analysis section HTML."""
    if not monte_carlo:
        return ""
    composite = monte_carlo.get("composite", {})
    if not composite:
        return ""

    cis = composite.get("confidence_intervals", {})
    ci95 = cis.get("95%", [0, 0])
    tail = composite.get("tail_risk", {})
    conv = composite.get("convergence", {})

    html = f"""
<h2>Probabilistic Analysis (Monte Carlo)</h2>
<div class="metric-grid">
    <div class="metric-card">
        <div class="metric-value">{composite.get('mean', 0):.1f}%</div>
        <div class="metric-label">Mean Favorability</div>
    </div>
    <div class="metric-card">
        <div class="metric-value">{composite.get('median', 0):.1f}%</div>
        <div class="metric-label">Median</div>
    </div>
    <div class="metric-card">
        <div class="metric-value">{ci95[0]:.1f}% - {ci95[1]:.1f}%</div>
        <div class="metric-label">95% Confidence Interval</div>
    </div>
    <div class="metric-card">
        <div class="metric-value">{tail.get('expected_shortfall', 0):.1f}%</div>
        <div class="metric-label">Tail Risk (CVaR 5%)</div>
    </div>
</div>
<p><em>Based on {composite.get('metadata', {}).get('iterations', 0):,} Monte Carlo iterations. 
Convergence: {'Yes' if conv.get('converged') else 'No'} 
(relative error: {conv.get('relative_error', 0):.4f}).
Confidence intervals represent the statistical spread given the model's uncertainty estimates.</em></p>
"""
    return html


def generate_historical_section(historical):
    """Generate historical precedents comparison HTML."""
    if not historical:
        return ""
    precedents = historical.get("precedents", [])
    if not precedents:
        return ""

    html = "<h2>Historical Precedents</h2>\n"
    html += '<table><tr><th>Event</th><th>Outcome</th><th>Timeline</th><th>Key Metric</th><th>Relevance</th></tr>\n'
    for p in precedents:
        rel_pct = int((p.get("relevance_score", 0)) * 100)
        html += (
            f'<tr><td><strong>{p.get("event", "")}</strong></td>'
            f'<td>{p.get("outcome", "")}</td>'
            f'<td>{p.get("timeline", "")}</td>'
            f'<td>{p.get("key_metric", "")}</td>'
            f'<td>{rel_pct}%</td></tr>\n'
        )
    html += "</table>\n"
    return html
...
def build_full_report(report_md, sim_data, actions, visuals_dir, output_path,
    ...
    cover = generate_cover_page(
        title=cover_title or "Predictive Stakeholder Analysis",
        subtitle=cover_subtitle or "A Multi-Agent Simulation Report",
        scenario=scenario,
        date_str=date_str
    )
    ...
    historical = ""
    if payload_v1:
        mc_section = generate_mc_section(payload_v1.get("monte_carlo"))
        action_plan = generate_action_plan_section(payload_v1.get("decision"))
        historical = generate_historical_section(payload_v1.get("historical_comparison"))
...
Confidence intervals represent the statistical spread given the model's own uncertainty estimates — 
they do not guarantee real-world outcomes.
...
if __name__ == "__main__":
    ...
    visuals_dir = sys.argv[3] if len(sys.argv) > 3 else os.path.join(_repo_root, "visuals")
    output_path = (
        sys.argv[4]
        if len(sys.argv) > 4
        else os.path.join(_reports_dir, "Glas_Intelligence_Predictive_Report.pdf")
    )
    ...
    actions_dir = os.path.normpath(
        os.path.join(_repo_root, "backend", "uploads", "simulations", sim_id)
    )
    ...
    payload_path = os.path.join(
        _repo_root, "backend", "uploads", "reports"
    )
    for d in os.listdir(payload_path) if os.path.isdir(payload_path) else []:
        pv1 = os.path.join(payload_path, d, "payload.v1.json")
        if os.path.isfile(pv1):
            try:
                with open(pv1, "r", encoding="utf-8") as f:
                    p = json.load(f)
                if p.get("simulation_id") == sim_id:
                    payload_v1 = p
                    break
    ...
    build_full_report(report_md, sim_data, actions, visuals_dir, output_path,
                     ...
                     custom_risks=custom_risks, payload_v1=payload_v1)
