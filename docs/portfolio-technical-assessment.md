...
> **Status update (2026-08-16):** the empty-sim tape described below has been
> **replaced** by the V11 recording (`sim_11ae92c2d1d8`): 24 rounds, **382 real
> actions** (268 twitter + 114 reddit), 0 CJK, shipped as the demo tape with
> `DEMO_SPEEDUP 256` (~90 s visitor traverse). The analysis below remains the
> correct reading of the *original* tape — and the reason the replacement
> exists. The empty-sim failure it documents is fixed at the root (key-type
> routing + zero-action guard), and the V11 tape is the proof.
...
**Today (committed tree, measured):** a `VITE_DEMO_MODE=1` build opens Home, shows the scenario picker and demo banner, and replays the full pipeline against the **V11 golden tape** — simulation completes with **382 real actions** across 24 rounds, the report streams section by section, and the zero-external-origins assertion holds (no Google Fonts, no Supabase, no third party). The demo e2e suite passes 2/2. The backend boots from a fresh checkout and runs 273/273 tests. What a visitor sees: the scenario picker → virtual-clock replay (~90 s at DEMO_SPEEDUP 256) → populated simulation timeline → rendered report with agent log → **canned agent interviews** (Step 5, `DEMO_MODE` branch serving persona-grounded Q&As with a visible "recorded response" marker). One honest caveat: the Step-5 interviews are canned (the live OASIS subprocess is gone in a replay) — the marker makes that explicit rather than hiding it.

### Good questions an interviewer will ask — with the answers to give

- **"Is the demo production-ready?"** — The replay engine is committed, tested, and passing e2e (2/2), and the backend is green (273/273). The demo ships the **V11 golden tape** (382 real actions across 24 rounds), Step-5 agent interviews are **shipped** (canned, clearly marked "recorded response"), and the whole traverse runs in ~90 s. The honest caveats: the interviews are canned rather than live (the OASIS subprocess can't exist in a static replay — the marker says so), and the frontend suite carries one known cold-import flake in the router test (fixed with a 60 s hook timeout).
