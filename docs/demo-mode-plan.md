...
**E. Step 5 cannot be replayed naively.** `/api/simulation/interview` calls `SimulationRunner.check_env_alive(simulation_id)` and 400s if the OASIS environment process isn't still resident in wait-for-command mode. → *Resolved for the live backend: it now answers interviews after the process has exited by rebuilding each agent from the recorded run (`backend/app/services/offline_interview.py`). **Resolved for the static demo (2026-09-25):** the tapes cannot hold Step 5, because the resolver keys on method + path + query and never on the request body, so one recorded answer would come back for every agent and question. `backend/scripts/build_demo_step5.py` instead records real answers once per golden tape into `frontend/public/demo/<scenario>/step5.json`: every agent's answer to three questions through the same reconstructed-interview path, and the report agent's answer to three questions from the report text (no graph tools, so no Zep spend). `frontend/src/demo/step5.js` serves `/interview/batch` and `/report/chat` from that file in the live shapes, and Step 5 shows the recorded questions as buttons; any other question gets a plain "this demo replays recorded answers only" reply. A scenario without `step5.json` falls back to the tape, as before. The frontend no longer asks `env-status` in demo mode. The backend `demo_interviews.py` (canned Q&A behind `Config.DEMO_MODE`) was removed: the static demo never reaches the backend, and nothing set `DEMO_MODE`.*
...
- *Step 5:* `POST /api/simulation/interview/batch` and `POST /api/report/chat` are answered from `step5.json`, not the tape (see §1.4 E); `env-status` is not called in demo mode
...
- **Step 5 answers come from `step5.json`, not the tape** (see §1.4 E). `9849885` also added a backend `demo_interviews.py` with canned Q&A behind `Config.DEMO_MODE`; the static demo never reaches the backend and nothing set the flag, so it was later removed.
