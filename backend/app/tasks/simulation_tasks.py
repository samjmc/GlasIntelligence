...
def prepare_simulation_task(self, simulation_id: str, task_id: str, graph_id: str,
                            user_plan: str = "pro"):
    ...
    try:
        ...
        task_manager.update_task(
            task_id, status=TaskStatus.COMPLETED, message="Simulation preparation complete",
            progress=100, result=result if isinstance(result, dict) else {"status": "complete"},
        )
        ...
        task_manager.update_task(
            task_id, status=TaskStatus.FAILED, message=f"Preparation failed: {e}", error=str(e),
        )
        raise


@celery_app.task(bind=True, name='glas.run_simulation', soft_time_limit=3600, time_limit=3900)
def run_simulation_task(self, simulation_id: str, platform: str = 'all',
                        max_rounds: int = 40, user_plan: str = "pro"):
    ...
    result = runner.start_simulation(simulation_id, platform=platform, max_rounds=max_rounds,
                                     user_plan=user_plan)
    return result
