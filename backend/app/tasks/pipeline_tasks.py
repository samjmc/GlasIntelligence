...
def run_full_pipeline(self, project_id: str, simulation_id: str, task_id: str,
                      graph_id: str, max_rounds: int = None, user_plan: str = "pro"):
    """
    ...
    from .report_tasks import generate_report_task, generate_pdf_task
    ...
    try:
        ...
        task_manager.update_task(
            task_id, status=TaskStatus.COMPLETED, message="Pipeline complete",
            progress=100, result={"project_id": project_id, "simulation_id": simulation_id},
        )
        ...
        task_manager.update_task(
            task_id, status=TaskStatus.FAILED, message=f"Pipeline failed: {e}", error=str(e),
        )
        raise
