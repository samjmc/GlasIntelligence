...
def generate_report():
    ...
    Use POST /api/report/generate/status to query progress.
    ...
    try:
        ...
        if not graph_id:
            return jsonify({"success": False, "error": "Missing graph ID, please ensure the graph has been built"}), 400
            ...
            try:
                ...
                if report.status == ReportStatus.COMPLETED:
                    ...
                    task_manager.complete_task(
                        task_id,
                        result={"report_id": report.report_id, "simulation_id": simulation_id, "status": "completed"},
                    )
                else:
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify(
                {"success": False, "error": f"No report found for simulation: {simulation_id}", "has_report": False}
            ), 404
...
def list_reports():
    ...
    try:
        ...
        reports = [
            r
            for r in ReportManager.list_reports(simulation_id=simulation_id, limit=None)
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({"success": False, "error": f"Report not found or progress unavailable: {report_id}"}), 404
...
@report_bp.route("/<report_id>/payload", methods=["GET"])
@require_auth
def get_report_payload(report_id: str):
    """Return structured report payload v1 (JSON) if present."""
    try:
        ...
        if not payload:
            return jsonify(
                {
                    "success": False,
                    "error": f"No payload.v1.json for report: {report_id}",
                }
            ), 404
        return jsonify({"success": True, "data": payload})
    except Exception as e:
        logger.error(f"Failed to load report payload: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_agent_log(report_id: str):
    ...
    Query parameters:
        from_line: Start reading from line N (optional, default 0, for incremental retrieval)
            ...
            scenarios.append(
                {
                    ...
                    "scenarios_tested": [s.get("name", "") for s in (payload.get("scenarios", []) or [])],
                }
            )
