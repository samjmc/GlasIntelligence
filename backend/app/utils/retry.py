...
class RetryableAPIClient:
    ...
    def __init__(
        self, max_retries: int = 3, initial_delay: float = 1.0, max_delay: float = 30.0, backoff_factor: float = 2.0
    ):
    ...
    def call_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                logger.warning(f"API call attempt {attempt + 1} failed: {str(e)}, retrying in {current_delay:.1f}s...")
    ...
    def call_batch_with_retry(
        ...
        Args:
            ...
            process_func: Processing function, receives a single item
            exceptions: Exception types to retry on
