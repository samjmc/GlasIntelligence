...
logger = get_logger("glas.simulation_ipc")
...
class SimulationIPCClient:
    ...
    def send_command(
        self, command_type: CommandType, args: dict[str, Any], timeout: float = 60.0, poll_interval: float = 0.5
    ) -> IPCResponse:
        """
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: Interview on both platforms per Agent in dual-platform mode
            timeout: Timeout in seconds
        ...
        return self.send_command(command_type=CommandType.BATCH_INTERVIEW, args=args, timeout=timeout)
...
class SimulationIPCServer:
    ...
    def _update_env_status(self, status: str):
        ...
        with open(status_file, "w", encoding="utf-8") as f:
            json.dump({"status": status, "timestamp": datetime.now().isoformat()}, f, ensure_ascii=False, indent=2)
