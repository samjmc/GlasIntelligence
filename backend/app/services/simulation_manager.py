        ...
        try:
            ...
            if len(filtered.entities) > max_agents:
                ...
                filtered.entities = sorted(
                    filtered.entities,
                    key=lambda e: len(getattr(e, "edges", []) or []),
                    reverse=True,
                )[:max_agents]
            ...
            if progress_callback:
                progress_callback("generating_profiles", 0, "Starting generation...", current=0, total=total_entities)
            ...
            if progress_callback:
                progress_callback(
                    "generating_profiles", 95, "Saving Profile files...", current=total_entities, total=total_entities
                )
