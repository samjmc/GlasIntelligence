"""
Action log recorder
Records the actions of every agent in an OASIS simulation for backend monitoring
...
Log structure:
    ...
    └── run_state.json       # Run state (used by the API)
"""
...
class PlatformActionLogger:
    """Per-platform action log recorder."""
...
class ActionLogger:
    """
    Action log recorder (backwards-compatible legacy interface)
...
# Global log instance (backwards-compatible legacy interface)
_global_logger: Optional[ActionLogger] = None


def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """Get the global log instance (backwards-compatible legacy interface)."""
