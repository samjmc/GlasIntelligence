...
This script runs the simulation using the parameters in the config file, fully automated

Features:
- Keeps the environment open after the simulation completes, entering wait-for-command mode
...
# Add the project path (shared with other runners — see scripts/lib/paths.py)
...
class MaxTokensWarningFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        return not ("max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage())


# Add the filter immediately at module load time so it takes effect before camel code runs
...
class TwitterSimulationRunner:
    ...
    # Twitter-available actions (INTERVIEW is excluded; it is triggered only via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        ...
        Uses the config from the .env file at the project root (highest priority):
        - LLM_API_KEY: API key
        ...
        # Set the environment variables camel-ai needs
        if llm_api_key:
    ...
    def _get_active_agents_for_round(
        ...
        for cfg in agent_configs:
            ...
            # Check whether we are in active hours
    ...
    async def run(self, max_rounds: int = None):
        ...
        Args:
            max_rounds: Maximum simulation rounds (optional, used to cap overly long simulations)
        """
        print("=" * 60)
        ...
        # Cap the rounds if a maximum was specified
        ...
        for round_num in range(total_rounds):
            ...
            # Get the agents active this round
        ...
        if self.wait_for_commands:
            ...
            # Wait-for-command loop (uses the global _shutdown_event)
            try:
...
async def main():
    ...
    # Initialize logging config (fixed file names, old logs cleaned)
...
def setup_signal_handlers():
    """
    Set up signal handlers so the process exits cleanly on SIGTERM/SIGINT
    giving the program a chance to clean up resources (close the database, environment, etc.)
    """
